...
func TestUrlStandardValues(t *testing.T) {
	...
	ctx = context.WithValue(ctx, Event, &models.Event{ID: 5})

	assert.Equal(t, "/teams/team-slug/tournaments/tournament-slug/players/player-slug/games/game-slug/events/5/url", Url(ctx, Team, Tournament, Player, Game, Event, "url"))
