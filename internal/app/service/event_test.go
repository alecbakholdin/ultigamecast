...
func TestEvent(t *testing.T) {
	...
	t.Run("general game flow works", func(t *testing.T) {
		...
		t.Run("02 start point defense", func(t *testing.T) {
			...
			u := <-sub.EventChan
			assert.Equal(t, u.Events[0].ID, u.Game.LastEvent.String)
			assert.Equal(t, models.GameLiveStatusOpponentPossession, u.Game.LiveStatus)
			assert.Equal(t, 1, len(u.Events))
			assert.Equal(t, models.EventTypePointStart, u.Events[0].Type)
			assert.Equal(t, int64(0), u.Events[0].TeamScore)
			assert.Equal(t, int64(0), u.Events[0].OpponentScore)
		})
		...
		t.Run("03 opponent drop", func(t *testing.T) {
			...
			assert.Equal(t, u.Id, u.Game.LastEvent.String)
			assert.Equal(t, models.EventTypeOpponentDrop, u.Events[0].Type)
			assert.Equal(t, int64(0), u.Events[0].TeamScore)
			assert.Equal(t, int64(0), u.Events[0].OpponentScore)
		})
		...
		t.Run("04 team drop", func(t *testing.T) {
			...
			u := <-sub.EventChan
			assert.Equal(t, 1, len(u.Events))
			assert.Equal(t, playerIds[0], u.Events[0].Player.Int64)
			assert.Equal(t, u.Id, u.Game.LastEvent.String)
			assert.Equal(t, models.EventTypeDrop, u.Events[0].Type)
			assert.Equal(t, models.GameLiveStatusOpponentPossession, u.Game.LiveStatus)
			assert.Equal(t, int64(0), u.Events[0].TeamScore)
			assert.Equal(t, int64(0), u.Events[0].OpponentScore)
		})
		...
		t.Run("05 opponent timeout", func(t *testing.T) {
			...
			assert.Equal(t, models.EventTypeOpponentTimeout, u.Events[0].Type)
			assert.Equal(t, int64(0), u.Events[0].TeamScore)
			assert.Equal(t, int64(0), u.Events[0].OpponentScore)
		})
		...
		t.Run("06 opponent end timeout", func(t *testing.T) {
			...
			assert.Equal(t, u.Id, u.Game.LastEvent.String)
			assert.Equal(t, models.GameLiveStatusOpponentPossession, u.Game.LiveStatus)
			assert.Equal(t, models.EventTypeOpponentTimeoutEnd, u.Events[0].Type)
			assert.Equal(t, int64(0), u.Events[0].TeamScore)
			assert.Equal(t, int64(0), u.Events[0].OpponentScore)
		})
		...
		t.Run("07 opponent turn", func(t *testing.T) {
			...
			assert.Equal(t, u.Id, u.Game.LastEvent.String)
			assert.Equal(t, models.GameLiveStatusTeamPossession, u.Game.LiveStatus)
			assert.Equal(t, models.EventTypeOpponentTurn, u.Events[0].Type)
			assert.Equal(t, int64(0), u.Events[0].TeamScore)
			assert.Equal(t, int64(0), u.Events[0].OpponentScore)
		})
		...
		t.Run("08 team timeout", func(t *testing.T) {
			...
			assert.Equal(t, models.EventTypeTimeout, u.Events[0].Type)
			assert.Equal(t, int64(0), u.Events[0].TeamScore)
			assert.Equal(t, int64(0), u.Events[0].OpponentScore)
		})
		...
		t.Run("09 team end timeout", func(t *testing.T) {
			...
			assert.Equal(t, u.Id, u.Game.LastEvent.String)
			assert.Equal(t, models.GameLiveStatusTeamPossession, u.Game.LiveStatus)
			assert.Equal(t, models.EventTypeTimeoutEnd, u.Events[0].Type)
			assert.Equal(t, int64(0), u.Events[0].TeamScore)
			assert.Equal(t, int64(0), u.Events[0].OpponentScore)
		})
		...
		t.Run("10 team goal", func(t *testing.T) {
			...
			u := <-sub.EventChan
			assert.Equal(t, 2, len(u.Events))
			assert.Equal(t, u.Id, u.Game.LastEvent.String)
			assert.Equal(t, models.GameLiveStatusPrePoint, u.Game.LiveStatus)
			assert.Equal(t, int64(1), u.Game.TeamScore)
			assert.Equal(t, int64(0), u.Game.OpponentScore)

			assert.Equal(t, models.EventTypeGoal, u.Events[0].Type)
			assert.Equal(t, playerIds[0], u.Events[0].Player.Int64)
			assert.Equal(t, int64(0), u.Events[0].TeamScore)
			assert.Equal(t, int64(0), u.Events[0].OpponentScore)
			assert.Equal(t, u.Id, u.Events[0].Batch.String)

			assert.Equal(t, models.EventTypeAssist, u.Events[1].Type)
			assert.Equal(t, playerIds[1], u.Events[1].Player.Int64)
			assert.Equal(t, int64(0), u.Events[1].TeamScore)
			assert.Equal(t, int64(0), u.Events[1].OpponentScore)
			assert.Equal(t, u.Id, u.Events[1].Batch.String)
		})
		...
		t.Run("11 opponent prepoint timeout", func(t *testing.T) {
			...
			assert.Equal(t, models.GameLiveStatusOpponentTimeout, u.Game.LiveStatus)
			assert.Equal(t, models.EventTypeOpponentTimeout, u.Events[0].Type)
			assert.Equal(t, int64(1), u.Events[0].TeamScore)
			assert.Equal(t, int64(0), u.Events[0].OpponentScore)
		})
		...
		t.Run("16 opponent goal", func(t *testing.T) {
			...
			assert.Equal(t, models.EventTypeOpponentGoal, u.Events[0].Type)
			assert.Equal(t, int64(1), u.Events[0].TeamScore)
			assert.Equal(t, int64(0), u.Events[0].OpponentScore)
		})
		...
		t.Run("17 start point offense", func(t *testing.T) {
			...
			assert.Equal(t, models.EventTypePointStart, u.Events[0].Type)
			assert.Equal(t, int64(1), u.Events[0].TeamScore)
			assert.Equal(t, int64(1), u.Events[0].OpponentScore)
		})
		...
		t.Run("18 team goal", func(t *testing.T) {
			...
			assert.Equal(t, models.EventTypeGoal, u.Events[0].Type)
			assert.Equal(t, playerIds[1], u.Events[0].Player.Int64)
			assert.Equal(t, int64(1), u.Events[0].TeamScore)
			assert.Equal(t, int64(1), u.Events[0].OpponentScore)
			assert.Equal(t, u.Id, u.Events[0].Batch.String)

			assert.Equal(t, models.EventTypeAssist, u.Events[1].Type)
			assert.Equal(t, playerIds[0], u.Events[1].Player.Int64)
			assert.Equal(t, int64(1), u.Events[0].TeamScore)
			assert.Equal(t, int64(1), u.Events[0].OpponentScore)
			assert.Equal(t, u.Id, u.Events[1].Batch.String)
		})
